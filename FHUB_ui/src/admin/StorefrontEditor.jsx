...
function CMSImage({ image, focus = 'center center', file, onFile, onUrl, onError, label }) {
  ...
  return <div className="cms-image"><img src={preview || image.url} alt={label} style={{ objectPosition: focus }} /><label className="cms-upload"><ImagePlus size={17} /> {file ? 'Change selected image' : 'Upload image'}<input aria-label={label} type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} /></label><div className="cms-url"><input aria-label={`${label} URL`} type="url" placeholder="Or paste an HTTPS image link" value={url} onChange={e => setUrl(e.target.value)} /><button type="button" onClick={useUrl} disabled={!url.trim()}>Use link</button></div></div>;
}
